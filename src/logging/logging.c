...
uint32 AV_LOG_LINE = AV_LOG_LINE_DEFAULT;
uint32 AV_LOG_FILE = AV_LOG_FILE_DEFAULT;
uint32 AV_LOG_FUNC = AV_LOG_FUNC_DEFAULT;
uint32 AV_LOG_PROJECT = AV_LOG_PROJECT_DEFAULT;
uint32 AV_LOG_TIME = AV_LOG_TIME_DEFAULT;
uint32 AV_LOG_TYPE = AV_LOG_TYPE_DEFAULT;
uint32 AV_LOG_ERROR = AV_LOG_ERROR_DEFAULT;
uint32 AV_LOG_COLORS = AV_LOG_COLORS_DEFAULT;
uint32 AV_LOG_ASSERT = AV_LOG_ASSERT_DEFAULT;
uint32 AV_LOG_CODE = AV_LOG_CODE_DEFAULT;
uint32 AV_LOG_CATEGORY_ = AV_LOG_CATEGORY_DEFAULT;
uint32 blacklistedCategoryCount = 0;
const char** blacklistedCategories = nullptr;
AvAssertLevel AV_ASSERT_LEVEL = AV_ASSERT_LEVEL_DEFAULT;

uint32 blacklistedMessageCount = 0;
const AvResult* blacklistedMessages = nullptr;


char AV_LOG_PROJECT_NAME[64] = "PROJECT_NAME_NOT_SPECIFIED";
uint32 AV_LOG_PROJECT_VERSION = 0;
...
const uint32 defaultDisabledLogCategoryCount = sizeof(defaulDisabledLogCategories) / sizeof(const char*);

const AvResult defaultDisabledMessages[] = {
};
const uint32 defaultDisabledMessageCount = sizeof(defaultDisabledMessages) / sizeof(AvResult);
...
bool32 checkCategoryDisabled(const char* category, AvResult result) {
	for (uint32 i = 0; i < blacklistedCategoryCount; i++) {
		if (strcmp(blacklistedCategories[i], category) == 0) {
			if ((uint32)result < (uint32)AV_LOG_LEVEL_INFO) {
				return true;
			}else{
				return false;
			}
...
bool32 checkMessageDisabled(AvResult result) {
	for (uint32 i = 0; i < blacklistedMessageCount; i++) {
		switch(blacklistedMessages[i]){
			case AV_DEBUG:
			case AV_INFO:
			case AV_SUCCESS:
			case AV_WARNING:
...
void printTags(AvResult result, AV_LOCATION_ARGS, AV_CATEGORY_ARGS, bool32 printArrow) {
	//message
	const char* message;

	char error_code[12] = { 0 };
	char error_type = 'S';
	uint32 error_num = result;
...
void avLog_(AvResult result, AV_LOCATION_ARGS, AV_CATEGORY_ARGS, const char* msg, ...) {
	...
	if ((uint32)result < (uint32)AV_LOG_LEVEL) {
		return;
	}
...
void avAssert_(AvResult result, AvResult valid, AV_LOCATION_ARGS, AV_CATEGORY_ARGS, const char* msg, ...) {
	...
	bool32 enabled = !checkCategoryDisabled(category, result) && !checkMessageDisabled(result);
